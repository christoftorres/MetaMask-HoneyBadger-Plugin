...
export default class SenderToRecipient extends PureComponent {
  ...
  renderRecipientWithAddress () {
    const { t } = this.context
    const {
      recipientEns, recipientName, recipientAddress, recipientNickname, addressOnly, onRecipientClick
    } = this.props
    const checksummedRecipientAddress = checksumAddress(recipientAddress)
      ...
      <div>
      {
        Object.keys(audit).length
          ? <div className={classnames({
            'audit-warn': audit.status === 'warning',
            'audit-approve': audit.status !== 'warning',
          })}>
            <div className="auditor">
              {`${audit.auditor} ${audit.status === 'warning' ? 'warning' : 'approval'}: `}
